...
async def on_user_join(client: Client, chat_member_updated):
    ...
    if new_chat_member.status == "member":
        ...
        # If the user joined the channel, remove their message count and unmute them
        try:
            user_member = await app.get_chat_member(channel_id, user_id)
            # If the user is now a member of the channel, unmute them and reset count
...
async def check_forcesub(client: Client, message: Message):
    ...
    try:
        ...
        if user_data:
            ...
            if message_count >= 3:
                # User has already sent 3 messages, should be muted
                return False
            else:
...
async def enforce_forcesub(client: Client, message: Message):
    if not await check_forcesub(client, message):
        # Delete the message if user is muted
        try:
